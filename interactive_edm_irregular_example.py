...
def onclick(event):
    ...
    y_pred,y_error = model.predict(x_test,return_std=True)
    #del ax[1].lines[:]
    #ax[1].plot(x_test[:,0].detach().numpy(),y_pred.detach().numpy(),color='orangered')
    #ax[1].fill_between(x_test[:,0].detach().numpy().flatten(),y_pred.detach().numpy().flatten()-y_error.detach().numpy().flatten(),y_pred.detach().numpy().flatten()+y_error.detach().numpy().flatten(),color='orangered',alpha=0.5)
    plt.draw()
    x_count += 1
